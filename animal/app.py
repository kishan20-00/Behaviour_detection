...
model_path = "sequence_model.h5"  # Update with the path to your model file
sequence_model = load_model(model_path)

train_df = pd.read_csv("F:/KA Projects/Thushani/animal/train.csv")
label_processor = keras.layers.StringLookup(num_oov_indices=0, vocabulary=np.unique(train_df["Text"]))
...
def predict():
    ...
    for i, class_label in enumerate(class_labels):
        predictions[class_label] = probabilities[i].item()
